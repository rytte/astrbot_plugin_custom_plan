...
| `todo` 待办列表 | 打卡业务使用现有打卡规则；其他计划需标题、状态字段及完成状态值 | 未完成/未达标优先，可按字段排序，支持分页 |
| `calendar` 日历 | 日期字段，可选标题或数值字段 | 月历；桌面版每天最多两条摘要并提示其余记录数量，移动版在月历下方按日期展示全部记录摘要，提示无日期记录 |
...
打卡记录带不可由模型伪造的作者。个人与受保护群计划的打卡面板显示创建者，公开群计划显示当前请求用户；统计区块仍按自己的范围计算。打卡面板和打卡式列表的右上角，个人计划不显示用户标签，群计划只显示统计对象的昵称，不加后缀。昵称从平台消息或群成员资料实时获取，不由模型传入、不写入计划；暂时获取不到时隐藏昵称，正常生成看板。

打卡业务使用 `view {type:"todo"}` 时，列表按已有记录的日期汇总数量，按当日生效阈值计算达标状态，用户范围与打卡面板一致。标题使用日期和计划名称，打卡预设的备注随记录展示；未记录日期不生成任务。列表按汇总后的天数分页，默认未达标优先、同组按日期升序；显式字段排序时按排序后记录中各日期首次出现的顺序展示。筛选与打卡面板相同，在汇总前应用。切换不修改字段、记录、业务规则或监督协议；不允许绑定独立的状态字段替代打卡结果。其他计划缺少待办字段绑定时返回具体缺失项，不自动创建字段或推断其含义。
