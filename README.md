...
| 视图 | 配置要求 | 展示方式 |
| --- | --- | --- |
| `table` 通用表格（默认） | 任意字段，也允许空数据集 | 每页最多 30 条记录、6 列，支持行列分别翻页 |
| `checkin` 打卡面板 | 打卡预设及其日期字段 | 近 28 天、当天数量、达标状态、连续天数 |
| `todo` 待办列表 | 标题、状态字段及完成状态值 | 未完成优先，可按字段排序，支持分页 |
| `calendar` 日历 | 日期字段，可选标题或数值字段 | 月历、每天两条摘要及其余记录数量，提示无日期记录 |
...
使用 `python scripts/preview.py --browser "浏览器可执行文件路径"` 可生成四种视图的示例图片，并输出冷启动与复用浏览器后的耗时。使用 `python scripts/package.py` 生成可导入的 ZIP，排除虚拟环境、测试数据和示例图片。
